...
```{r}
#grafen og veridene i df_totalindeks blir lavere enn de skal være
df_totalindeks %>%
...
Svært like grafer. Får ikke helt samme verdi på y-aksen, siden de konsumgruppene vi har brukt, kun er en undergruppe når det kommer til den fulle utregningen av KPI.
...
delta_P <- as.Date("2011-01-01")-as.Date("2010-01-01")
```

```{# Besvar oppgave IV her}
#får ikke til
merge(df_API, df_totalindeks) %>%
  ggplot(aes(x=dato))+
  geom_line(aes(y = `Elektrisitet inkludert nettleie`, color= "red"))+
  geom_line(aes(y = `12-måneders endring (prosent)`, color= "blue"))+
  labs(x="dato", y="prosent",
         title="Vekstbidraget til konsumgruppene",
         subtitle="Hva er vekstbidraget til konsumgruppen Elektrisitet, inkludert nettleie?", color = "")+
  scale_y_continuous(labels = scales::comma)+
  guides(color = "none")+
  theme_bw()
```
